...
var Resource = /** @class */ (function () {
    ...
    Resource.prototype.cacheDdlLog = function (_a) {
        ...
        var newCachedDdlLogs = __spreadArray(__spreadArray([], ddlLogs, true), cachedDdlLogs, true).filter(function (log, index, self) {
            return index === self.findIndex(function (t) { return (t.logIndex === log.logIndex); });
        });
                ...
                switch (_e.label) {
                    ...
                    case 1:
                        ...
                        _loop_1 = function (i) {
                            ...
                            var _g = pairInfo.token0.adr === baseToken
                                ? [constant_1.POOL_IDS.token0, constant_1.POOL_IDS.token1]
                                : [constant_1.POOL_IDS.token1, constant_1.POOL_IDS.token0], baseId = _g[0], quoteId = _g[1];
    ...
    Resource.prototype.parseDdlLogs = function (ddlLogs) {
        ...
        return ddlLogs.map(function (log) {
            try {
                return __assign({ address: log.address, timeStamp: Number(log.timeStamp), transactionHash: log.transactionHash, blockNumber: log.blockNumber, index: log.logIndex, logIndex: log.transactionHash + '-' + log.logIndex }, eventInterface.parseLog(log));
            }
            catch (e) {
                return {};
            }
