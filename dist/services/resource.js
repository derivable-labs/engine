...
var PoolOverride_json_1 = __importDefault(require("../abi/PoolOverride.json"));
...
var Resource = /** @class */ (function () {
    ...
    Resource.prototype.getLastBlockCached = function (account) {
        ...
        var lastWalletBlock = constant_1.ddlGenesisBlock[this.chainId] - 1;
        var walletBlockCached = this.storage.getItem(this.chainId + '-' + constant_1.LOCALSTORAGE_KEY.SWAP_BLOCK_LOGS + '-' + account);
    ...
    Resource.prototype.cacheDdlLog = function (_a) {
        ...
        var newCachedDdlLogs = __spreadArray(__spreadArray([], ddlLogs, true), cachedDdlLogs, true).filter(function (log, index, self) {
            return index === self.findIndex(function (t) { return (t.logIndex === log.logIndex && t.transactionHash === log.transactionHash); });
        });
    ...
    Resource.prototype.getNewResource = function (account) {
        return __awaiter(this, void 0, void 0, function () {
            var etherscanConfig, provider, lastHeadBlockCached, accTopic, topics, filterTopics;
            var _this = this;
    ...
    Resource.prototype.generatePoolData = function (logs) {
        ...
        logs.forEach(function (log) {
            if (log.name === 'PoolCreated') {
                var logic = ethers_1.ethers.utils.getAddress(log.topics[3].slice(0, 42));
                var factory = ethers_1.ethers.utils.getAddress(log.topics[2].slice(0, 42));
                var data = log.args;
                var powers = (0, helper_1.decodePowers)(ethers_1.ethers.utils.hexZeroPad(log.args.powers.toHexString(), 32));
                data.dTokens = powers.map(function (value, key) {
                    return { power: value, index: key };
                });
                data.dTokens = data.dTokens
                    .map(function (data) { return "".concat(log.address, "-").concat(data.index); });
                poolData[log.address] = __assign(__assign({}, data), { poolAddress: log.address, logic: logic, factory: factory, powers: powers, cToken: data.pairToken });
                allUniPools.push(data.pairToken);
                allTokens.push.apply(allTokens, __spreadArray(__spreadArray([], data.dTokens, false), [data.pairToken, data.baseToken], false));
            }
        });
            ...
            return __generator(this, function (_e) {
                switch (_e.label) {
                    ...
                    case 1:
                        ...
                        _d = this.parseMultiCallResponse(results, Object.keys(listPools)), tokensArr = _d.tokens, poolsState = _d.poolsState;
                        tokens = [];
    ...
    Resource.prototype.getPoolOverridedProvider = function (poolAddresses) {
        var stateOverride = {};
        ...
        //@ts-ignore
        this.overrideProvider.setStateOverride(__assign({}, stateOverride));
        return this.overrideProvider;
    };
    Resource.prototype.getBasePrice = function (pairInfo, baseToken) {
        var token0 = pairInfo.token0.adr;
        var r0 = pairInfo.token0.reserve;
        var r1 = pairInfo.token1.reserve;
        var _a = token0 === baseToken ? [r0, r1] : [r1, r0], rb = _a[0], rq = _a[1];
    ...
    Resource.prototype.parseMultiCallResponse = function (multiCallData, poolAddresses) {
        var pools = {};
        var tokens = multiCallData.tokens.callsReturnContext[0].returnValues;
        poolAddresses.forEach(function (poolAddress) {
            var abiInterface = new ethers_1.ethers.utils.Interface(PoolOverride_json_1["default"].abi);
            var poolStateData = multiCallData['pools-' + poolAddress].callsReturnContext;
            var data = (0, helper_1.formatMultiCallBignumber)(poolStateData[0].returnValues);
            var encodeData = abiInterface.encodeFunctionResult('getStates', [data]);
            var formatedData = abiInterface.decodeFunctionResult('getStates', encodeData);
            pools[poolStateData[0].reference] = __assign({ twapBase: formatedData.states.twap.base._x, twapLP: formatedData.states.twap.LP._x, spotBase: formatedData.states.spot.base._x, spotLP: formatedData.states.spot.LP._x }, formatedData.states);
        });
    ...
    Resource.prototype.parseDdlLogs = function (ddlLogs) {
        ...
        return ddlLogs.map(function (log) {
            try {
                var decodeLog = eventInterface.parseLog(log);
                var appName = null;
                try {
                ...
                var lastHeadBlockCached = _this.getLastBlockCached(_this.account || '');
                return {
                    address: log.address,
