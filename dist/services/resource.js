...
class Resource {
    ...
    async fetchResourceData(account) {
        let result = {};
        if (!this.chainId)
            return result;
    ...
    async getResourceCached(account) {
        const results = {
            pools: {},
    ...
    async getPrice(pool, blockNumber, pair) {
        ...
        const twap = await OracleSdk.getPrice(getStorageAt, getBlockByNumber, pool.pair, pool.quoteTokenIndex, (0, helper_1.bn)(blockNumber).sub(Math.floor(pool.window.toNumber() / 2)).toNumber());
        let spot;
        const [r0, r1] = [pair.token0.reserve, pair.token1.reserve];
        if (pool.quoteTokenIndex == 0) {
            spot = r0.mul(exports.Q128).div(r1);
        }
        else {
            spot = r1.mul(exports.Q128).div(r0);
        }
        return {
            poolAddress: pool.poolAddress,
            twap: twap.mul((0, helper_1.bn)(2).pow(16)),
