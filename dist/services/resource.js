...
class Resource {
    ...
    calcPoolInfo(pool) {
        ...
        const dailyInterestRate = 1 - Math.pow(2, -SECONDS_PER_DAY / pool.INTEREST_HL.toNumber());
        return {
            riskFactor,
            deleverageRiskA,
            deleverageRiskB,
        ...
        return ddlLogs.map((log) => {
            try {
                ...
                if (appName === 'PoolCreated') {
                    const poolCreatedData = utils_1.defaultAbiCoder.decode(constant_1.EventDataAbis[appName], decodeLog.args.data);
