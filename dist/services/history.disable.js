...
class History {
    ...
    formatSwapHistory({ logs, poolAddress, states, powers, }) {
        try {
            ...
            const logGrouped = Object.values(lodash_1.default.groupBy(logs, (log) => log.transactionHash)).filter((order) => {
                return order.find((log) => ['TransferSingle', 'TransferBatch'].includes(log.args.name));
            });
            const orders = logGrouped
                .slice()
                .sort((a, b) => a[0].blockNumber - b[0].blockNumber);
            ...
            orders.forEach((txs) => {
                ...
                for (const tx of txs) {
                    if (tx.args.name === 'Transfer') {
                        ...
                        if (formatedData.from === this.account) {
                            balances[id] = balances[id]
                                ...
                                : (0, helper_1.bn)(0).sub(formatedData.value);
                        }
                        else if (formatedData.to === this.account) {
                            balances[id] = balances[id]
                                ? balances[id].add(formatedData.value)
                                : (0, helper_1.bn)(0).add(formatedData.value);
                        }
                        continue;
                    }
                    if (tx.args.name === 'TransferSingle') {
                        ...
                        const formatedData = ethers_1.ethers.utils.defaultAbiCoder.decode([
                            'address operator',
                            'address from',
                            'address to',
                            'uint256 id',
