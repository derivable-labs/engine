...
exports["default"] = {
    history: history,
    getBars: function (_a) {
        var route = _a.route, resolution = _a.resolution, inputToken = _a.inputToken, outputToken = _a.outputToken, limit = _a.limit, chainId = _a.chainId, to = _a.to;
        console.log(route);
        var q = route.split('/').join(',');
        var url = "".concat(constant_1.CHART_API_ENDPOINT.replace('{chainId}', chainId || '56'), "candleline4?q=").concat(q, "&r=").concat(convertResolution(resolution), "&l=").concat(limit, "&t=").concat(to);
