...
export class Simulate {
  ...
  exactIn(price: BigNumber, sideIn: number, amountIn: BigNumber, sideOut: number) {
  ...
  getReserveIn({amountIn, rV, sV} : {amountIn: BigNumber, rV: BigNumber, sV: BigNumber}) {
    // dsV = amountIn, rIn = drV = dsV * rV / sV
    return amountIn.mul(rV).sub(sV)
  }

  dsv({price, R1, rV, sV}: { price: BigNumber, R1: BigNumber, rV: BigNumber, sV: BigNumber }) {
