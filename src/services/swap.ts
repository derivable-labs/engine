...
export class Swap {
  ...
  constructor(account: string, config = DEFAULT_CONFIG) {
    if (
      ...
      throw new Error(
        `required chainId scanApi nativeToken rpcToGetLogs to be defined!`,
      )
    }

    if (
      !config.addresses.router ||
      !config.addresses.wrapToken ||
      !config.addresses.stateCalHelper
    ) {
      throw new Error(`required router wrapToken stateCalHelper to be defined!`)
    }
    ...
    this.signer = signer
    this.CURRENT_POOL = new CurrentPool(account, config)
  ...
  async calculateAmountOuts(steps: StepType[]) {
    ...
    try {
      ...
      const router = this.contractAddress.router
      // @ts-ignore
  ...
  convertStepForPoolErc1155(steps: SwapStepType[]): {
    ...
    let value = bn(0)
    steps.forEach((step) => {
      if (step.tokenIn === this.nativeToken) {
  ...
  async convertStepToActions(
    ...
    steps.forEach((step) => {
      if (
        (step.tokenIn === this.nativeToken ||
          step.tokenOut === this.nativeToken) &&
        this.CURRENT_POOL.TOKEN_R !== this.contractAddress.wrapToken
      ) {
      ...
      if (step.tokenOut === this.nativeToken) {
      ...
      if (
        ...
        metaDatas.push({
          code: this.contractAddress.stateCalHelper,
          inputs,
        })
        promises.push(
  ...
  getAddressByErc1155Address(address: string) {
    ...
    if (
      ...
      return this.contractAddress.wrapToken
    }
  ...
  getRouterContract(provider: any) {
    return new ethers.Contract(this.contractAddress.router, UtrAbi, provider)
  }
  ...
  getWrapContract(provider?: any) {
    return new ethers.Contract(
      this.contractAddress.wrapToken,
      WtapAbi,
      provider || this.provider,
    )
  }
